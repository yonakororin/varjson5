...
// Build: cmake + make  (see CMakeLists.txt)
...
static void dump(const Value& v, std::ostream& out, int indent, int depth) {
    ...
    auto pad  = [&](int d) { if (indent > 0) { int n = d * indent; for (int i = 0; i < n; ++i) out << ' '; } };
...
static std::string apply_vars(const std::string& s, const std::map<std::string, Value>& vars) {
    ...
    while (i < s.size()) {
        ...
        size_t end = s.find("}}", open + 2);
        if (end == std::string::npos) {
            out.append(s, open, s.size() - open);
            break;
        }
        size_t ka = open + 2, kb = end;
        while (ka < kb && (s[ka] == ' ' || s[ka] == '\t')) ++ka;
        while (kb > ka && (s[kb-1] == ' ' || s[kb-1] == '\t')) --kb;
        auto it = vars.find(s.substr(ka, kb - ka));
        if (it != vars.end()) {
            const Value& v = it->second;
            if (v.is_string()) out += v.as_string();
            else out += to_json(v, true);
        } else {
            out.append(s, open, end + 2 - open);
        }
        i = end + 2;
    }
    return out;
...
static std::map<std::string, Value> resolve_vars(std::map<std::string, Value> vars) {
    for (int pass = 0; pass < 100; ++pass) {
        ...
        for (auto& [k, v] : vars) {
            ...
            std::string after = apply_vars(v.as_string(), vars);
            if (after != v.as_string()) { v = Value(std::move(after)); changed = true; }
        }
        if (!changed) break;
    }
    return vars;
...
int varjson5_main_impl(int argc, char* argv[]) {
